...
for c in range(0,4):
  ...
  for i in range(len(points[c])):
    ...
    draw.ellipse((points[c][i][0]-0.00673/2*width, points[c][i][1], points[c][i][0]+0.00673/2*width, points[c][i][1]+0.0134/2*width), fill=None, outline=255)
  draw.line(points[c], fill=255, width=0)
